	...
	if (isLoading) {
		return (
			<section id="experience" className="bg-[#0b0a27] text-white">
				<h1 className="ml-[20px] xl:ml-[100px] text-[16px] sm:text-[20px] xl:text-[26px] 2xl:text-[36pX] italic uppercase font-bold py-5 mt-0">experience</h1>
				<hr></hr>
				<div>
		...
		<section id="experience" className="bg-[#0b0a27] text-white">
          ...
          <a href="/experience" target="_blank" rel="noopener noreferrer" className=" font-Code-Pro hover:text-[#caff00]  italic flex text-[26px] ml-3">
			...
			{data?.map((experience: any) => {
				...
				const formattedFrom = `${fromDate.getFullYear()}年 ${months[fromDate.getMonth()]}`;

				const toDate = experience.until ? new Date(experience.until) : null;
				const formattedUntil = toDate ? `${toDate.getFullYear()}年 ${months[toDate.getMonth()]}` : "今まで";
							...
							<div className="flex mb-10">
								...
								<div className="flex">
									<div className="flex flex-col my-auto">
										...
										<p className="font-Code-Pro text-base xl:text-lg 2xl:text-xl my-1">
											{experience.work_loc}
											<span className="text-[#caff00]"> にて</span>
										</p>
